$(function (){
    ...
    //添加事件监听
    $('#department-select').change(function (event){
        //双重否定去定位被选中的option
    ...
    $('#position-select').change(function (event){
        ...
        console.log("被选中的职位id"+positionId);
        //初始化分页插件
        flag = true;
        request_condition.current = 1;
    ...
    //对.employee-wrap列表下所有的a标签加点击事件,同时定位到class为employee-status-btn的a标签
    $('.employee-wrap').on('click','a',function (event){
        console.log(event.currentTarget.dataset)
        var target = $(event.currentTarget);
        //定位到是否是修改状态的a标签
        if (target.hasClass('employee-status-btn')){
            console.log('定位到了修改状态的a标签')
            let emId = event.currentTarget.dataset.id;
